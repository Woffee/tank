...
T_Map::~T_Map(void)
{
	vector<vector<int>>::iterator p = layer_date.begin();
	for (; p != layer_date.end();)
	{
		p->clear();
		p = layer_date.erase(p);
	}
	layer_date.clear();
	layer_date.swap(vector<vector<int>>());
	delete graph;
	graph = NULL;
		...
		if (Visible)
		{
			...
			for (r = 0, tY = 0; r < map_rows; r++, tY += tile_height)
			{
				for (c = 0, tX = 0; c < map_cols; c++, tX += tile_width)
				{
					...
					AlphaBlend(dc_buf,tX,tY,tile_width,tile_height,memDC,img_col*tile_width,img_row*tile_height,tile_width,tile_height,frame_bf);
				}
			}
...
void T_Map::Draw(HDC hdc)
{
	...
	if (tile_width > 0 && tile_height > 0)
	{
		...
		else
		{
			...
			frame_bf.SourceConstantAlpha = 255;
			frame_bf.AlphaFormat = AC_SRC_ALPHA;
			AlphaBlend(hdc,(int)X,(int)Y,(int)GetWidth(),(int)GetHeight(),dc_buf,0,0,(int)GetWidth(),(int)GetHeight(),frame_bf);
		}
